...
__export(schema_exports, {
  ...
  clientPurchasesRelations: () => clientPurchasesRelations,
  clients: () => clients,
...
var badgeEnum = pgEnum("badge_type", ["NEW", "BESTSELLER", "LIMITED", "SALE"]);
var purchaseStatusEnum = pgEnum("purchase_status", ["pending", "completed", "refunded", "cancelled"]);
...
var clientPurchases = pgTable("client_purchases", {
  id: serial("id").primaryKey(),
...
var clientPurchasesRelations = relations(clientPurchases, ({ one }) => ({
  client: one(clients, {
...
router.get("/", async (c) => {
  ...
  if (category) {
    ...
    }).from(products).leftJoin(categories, eq(products.categoryId, categories.id)).where(
      and(
    ...
    rows = await db.select({
      ...
      categoryName: categories.name,
      categorySlug: categories.slug
    }).from(products).leftJoin(categories, eq(products.categoryId, categories.id)).where(conditions.length > 0 ? and(...conditions) : void 0).limit(Number(limit)).offset(Number(offset));
  }
...
router2.get("/:slug/products", async (c) => {
  const slug = c.req.param("slug");
  const [category] = await db.select().from(categories).where(eq(categories.slug, slug)).limit(1);
  if (!category) {
    return c.json({ error: "Category not found" }, 404);
  }
  const productRows = await db.select().from(products).where(eq(products.categoryId, category.id));
...
router3.post("/", async (c) => {
  ...
  const result = await import_cloudinary.v2.uploader.upload(dataUri, {
...
router3.delete("/:publicId", async (c) => {
...
var categorySchema = external_exports.object({
  ...
  slug: external_exports.string().min(1).regex(/^[a-z0-9-]+$/, "Slug must be lowercase letters, numbers, and hyphens only"),
  description: external_exports.string().optional()
});
var productSchema = external_exports.object({
  categoryId: external_exports.number().int().positive().optional().nullable(),
  name: external_exports.string().min(1),
  slug: external_exports.string().min(1).regex(/^[a-z0-9-]+$/),
  description: external_exports.string().optional(),
  shortDescription: external_exports.string().optional(),
  price: external_exports.string().regex(/^\d+(\.\d{1,2})?$/, 'Price must be a decimal number e.g. "295.00"'),
  compareAtPrice: external_exports.string().regex(/^\d+(\.\d{1,2})?$/).optional().nullable(),
  images: external_exports.array(external_exports.string().url()).default([]),
  fabric: external_exports.string().optional(),
  embroideryType: external_exports.string().optional(),
  colors: external_exports.array(external_exports.string()).default([]),
  sizes: external_exports.array(external_exports.string()).default([]),
  badge: external_exports.enum(["NEW", "BESTSELLER", "LIMITED", "SALE"]).optional().nullable(),
  inStock: external_exports.boolean().default(true),
  stockQuantity: external_exports.number().int().min(0).default(0),
...
var clientSchema = external_exports.object({
  ...
  measurements: external_exports.record(external_exports.unknown()).optional().default({}),
  stylePreferences: external_exports.array(external_exports.string()).optional().default([]),
  notes: external_exports.string().optional().nullable(),
  tags: external_exports.array(external_exports.string()).optional().default([]),
  source: external_exports.string().optional().nullable()
});
var purchaseSchema = external_exports.object({
  productId: external_exports.number().int().positive().optional().nullable(),
  productName: external_exports.string().optional(),
  unitPrice: external_exports.string().regex(/^\d+(\.\d{1,2})?$/).optional(),
  quantity: external_exports.number().int().positive().default(1),
  totalAmount: external_exports.string().regex(/^\d+(\.\d{1,2})?$/).optional(),
  status: external_exports.enum(["pending", "completed", "refunded", "cancelled"]).default("completed"),
  notes: external_exports.string().optional()
});
router4.get("/categories", async (c) => {
  const rows = await db.select().from(categories).orderBy(categories.name);
  return c.json({ data: rows });
});
router4.post("/categories", zValidator("json", categorySchema), async (c) => {
  const data = c.req.valid("json");
  const [row] = await db.insert(categories).values(data).returning();
  return c.json({ data: row }, 201);
});
router4.put("/categories/:id", zValidator("json", categorySchema.partial()), async (c) => {
...
router4.delete("/categories/:id", async (c) => {
  const id = Number(c.req.param("id"));
  await db.delete(categories).where(eq(categories.id, id));
  return c.json({ success: true });
});
router4.get("/products", async (c) => {
  const rows = await db.select().from(products).orderBy(products.createdAt);
  return c.json({ data: rows });
});
router4.get("/products/:id", async (c) => {
  const id = Number(c.req.param("id"));
  const [row] = await db.select().from(products).where(eq(products.id, id));
  if (!row) return c.json({ error: "Product not found" }, 404);
  return c.json({ data: row });
});
router4.post("/products", zValidator("json", productSchema), async (c) => {
  const data = c.req.valid("json");
  const [row] = await db.insert(products).values(data).returning();
  return c.json({ data: row }, 201);
});
router4.put("/products/:id", zValidator("json", productSchema.partial()), async (c) => {
  const id = Number(c.req.param("id"));
  const data = c.req.valid("json");
  const [row] = await db.update(products).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(products.id, id)).returning();
  if (!row) return c.json({ error: "Product not found" }, 404);
  return c.json({ data: row });
});
router4.delete("/products/:id", async (c) => {
  const id = Number(c.req.param("id"));
  const [row] = await db.update(products).set({ inStock: false, stockQuantity: 0, updatedAt: /* @__PURE__ */ new Date() }).where(eq(products.id, id)).returning();
  if (!row) return c.json({ error: "Product not found" }, 404);
  return c.json({ data: row });
});
router4.get("/clients", async (c) => {
  const { search, tag, limit = "50", offset = "0" } = c.req.query();
  let rows;
  if (search) {
    rows = await db.select().from(clients).orderBy(clients.createdAt).limit(Number(limit)).offset(Number(offset));
  } else {
    rows = await db.select().from(clients).orderBy(clients.createdAt).limit(Number(limit)).offset(Number(offset));
  }
  return c.json({ data: rows });
});
router4.get("/clients/:id", async (c) => {
  const id = Number(c.req.param("id"));
  const [client] = await db.select().from(clients).where(eq(clients.id, id)).limit(1);
  if (!client) return c.json({ error: "Client not found" }, 404);
  const purchases = await db.select().from(clientPurchases).where(eq(clientPurchases.clientId, id)).orderBy(clientPurchases.purchaseDate);
  return c.json({ data: { ...client, purchases } });
});
router4.post("/clients", zValidator("json", clientSchema), async (c) => {
  const data = c.req.valid("json");
  const [row] = await db.insert(clients).values(data).returning();
  return c.json({ data: row }, 201);
});
router4.put("/clients/:id", zValidator("json", clientSchema.partial()), async (c) => {
  const id = Number(c.req.param("id"));
  const data = c.req.valid("json");
  const [row] = await db.update(clients).set({ ...data, updatedAt: /* @__PURE__ */ new Date() }).where(eq(clients.id, id)).returning();
  if (!row) return c.json({ error: "Client not found" }, 404);
  return c.json({ data: row });
});
router4.delete("/clients/:id", async (c) => {
  const id = Number(c.req.param("id"));
  await db.delete(clients).where(eq(clients.id, id));
  return c.json({ success: true });
});
router4.post("/clients/:id/purchases", zValidator("json", purchaseSchema), async (c) => {
  const clientId = Number(c.req.param("id"));
  const data = c.req.valid("json");
  let productName = data.productName;
  let unitPrice = data.unitPrice;
  if (data.productId && !productName) {
    const [product] = await db.select({ name: products.name, price: products.price }).from(products).where(eq(products.id, data.productId)).limit(1);
    if (product) {
      productName = product.name;
      unitPrice = unitPrice ?? product.price;
    }
  }
  const totalAmount = data.totalAmount ?? (unitPrice ? String(Number(unitPrice) * data.quantity) : void 0);
  const [row] = await db.insert(clientPurchases).values({ ...data, clientId, productName, unitPrice, totalAmount }).returning();
  return c.json({ data: row }, 201);
});
...
async function handler(req, res) {
  ...
  const response = await app_default.fetch(fetchReq);
